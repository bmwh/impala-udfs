...
// Based on the sample HyperLogLog UDA from the Cloudera UDF/UDA samples
// library:
// https://github.com/cloudera/impala-udf-samples
//
// Extended to handle string values, including running the HLL over
// delimited values within a single string
//
// From hyperloglog-uda.cc:
...
void StringHllInit(FunctionContext* ctx, StringVal* dst) {
...
void StringHllUpdate(FunctionContext* ctx, const StringVal& src, StringVal* dst) {
  ...
  size_t lpos = 0;
  while (true) {
    if (pos == src.len || src.ptr[pos] == '!') {
      uint64_t hash_value = FnvHash(src.ptr+lpos, pos-lpos, FNV64_SEED);
      if (hash_value != 0) {
...
void StringHllMerge(FunctionContext* ctx, const StringVal& src, StringVal* dst) {
  assert(dst != NULL);
  assert(!dst->is_null);
  assert(!src.is_null);
  assert(dst->len == pow(2, HLL_PRECISION));
...
const StringVal StringHllSerialize(FunctionContext* ctx, const StringVal& src) {
...
StringVal StringHllFinalize(FunctionContext* ctx, const StringVal& src) {
  assert(!src.is_null);
  assert(src.len == pow(2, HLL_PRECISION));
